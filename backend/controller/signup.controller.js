...
async function deleteAcc(req,res){
    try{
        const {id}=req.params;
        const user=await signModel.findByIdAndDelete(id);
        res.status(200).json({"message":`user ${id} is deleted`});
    }
    catch(error){
...
module.exports = { saveUser, checkUser, updatePassword, getUserData, updateUserProfile,deleteAcc};